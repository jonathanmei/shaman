...
def probe_key(cfg: dict) -> str:
    """Key of the calibration-time rank-sensitivity probe (calibration key + the probe's ADMM inputs)."""
    return hash_json({
        "stats": stats_key(cfg),
        "fields": _subset(cfg, PROBE_FIELDS),
        "code": fingerprint_sources("rank_probe"),
    })
